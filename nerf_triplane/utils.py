...
class LMDMeter:
    def __init__(self, backend='dlib', region='mouth'):
        ...
        if self.backend == 'dlib':
            ...
            # MediaPipe to 68-landmark mapping (approximate)
            # These indices map MediaPipe's 468 landmarks to standard 68 landmarks
            self.mediapipe_to_68 = self._get_mediapipe_mapping()
    ...
    def _get_mediapipe_mapping(self):
        """
        Map MediaPipe's 468 face landmarks to standard 68 landmarks.
        Returns indices for approximate mapping.
        """
        # Approximate mapping from MediaPipe to 68-point landmarks
        # Focus on key facial features, especially mouth region
        mapping = {
            # Mouth outer (48-59)
            48: 61, 49: 185, 50: 40, 51: 39, 52: 37, 53: 0, 54: 267, 55: 269, 56: 270, 57: 409, 58: 291, 59: 375,
            # Mouth inner (60-67)
            60: 78, 61: 191, 62: 80, 63: 81, 64: 13, 65: 311, 66: 310, 67: 415,
        }
        return mapping
    ...
    def get_landmarks(self, img):
        ...
        if self.backend == 'dlib':
            ...
            # Extract landmarks for mouth region (or map all 68 if needed)
            if self.region == 'mouth':
                # Extract only mouth landmarks (20 points)
                lms = np.zeros((20, 2), dtype=np.float32)
                for i, mp_idx in enumerate(range(48, 68)):
                    if mp_idx in self.mediapipe_to_68:
                        landmark = face_landmarks.landmark[self.mediapipe_to_68[mp_idx]]
                        lms[i, 0] = landmark.x * w
                        lms[i, 1] = landmark.y * h
            else:
                # For full face, return all available mouth landmarks
                lms = np.zeros((20, 2), dtype=np.float32)
                for i, mp_idx in enumerate(range(48, 68)):
                    if mp_idx in self.mediapipe_to_68:
                        landmark = face_landmarks.landmark[self.mediapipe_to_68[mp_idx]]
            ...
            result = self.predictor.get_landmarks(img)
    ...
    def update(self, preds, truths):
        ...
        # Skip this sample if landmarks cannot be detected
    ...
    def measure(self):
        if self.N == 0:
            return 0.0  # Return 0 if no valid samples were processed
        return self.V / self.N
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    max_aud = np.max(pred_ambient_aud)
                    if max_aud > 1e-8:  # Avoid division by zero or very small numbers
                        pred_ambient_aud /= max_aud
                    
                    pred_ambient_eye = pred_ambient_eye[0].detach().cpu().numpy()
                    max_eye = np.max(pred_ambient_eye)
                    if max_eye > 1e-8:  # Avoid division by zero or very small numbers
                        pred_ambient_eye /= max_eye
                    ...
                    max_unc = np.max(pred_uncertainty)
                    if max_unc > 1e-8:  # Avoid division by zero or very small numbers
                        pred_uncertainty /= max_unc
