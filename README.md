...
This repository provides a clean API for running inference with the NeRF-Triplane model.
...
You can run inference using the command line interface:

```bash
python main.py data/May --workspace model/trial_may --asr_model ave --portrait
```

Arguments:
- `path`: Path to the data directory (required)
- `--workspace`: Path to the workspace directory (default: 'workspace')
- `--asr_model`: ASR model to use ('ave' or 'deepspeech', default: 'ave')
- `--portrait`: Only render face (flag)
...
```python
from nerf_triplane.api import NeRFAPI
...
# Load checkpoint
api.load_checkpoint()  # Uses latest checkpoint by default
# Or specify a checkpoint:
# api.load_checkpoint('path/to/checkpoint.pth')

# Run inference
test_loader = api.prepare_test_data()
results = api.inference(test_loader)
```
...
The main API class that handles model initialization and inference.
...
- `load_checkpoint(checkpoint_path: Optional[str] = None)`: Load a model checkpoint
- `prepare_test_data() -> NeRFDataset`: Prepare the test dataset
- `inference(test_loader: Optional[NeRFDataset] = None) -> Dict[str, Any]`: Run inference on the test dataset
