...
def face_tracking(ori_imgs_dir, use_deca=False, use_landmark_refinement=False, landmark_method='mediapipe', gpu_id=0):

    print(f'[INFO] ===== perform face tracking =====')
    
    if use_deca:
        if use_landmark_refinement:
            print(f'[INFO] Using DECA + {landmark_method} landmark refinement on GPU {gpu_id}')
        else:
            print(f'[INFO] Using DECA without landmark refinement (direct encoding) on GPU {gpu_id}')
    else:
        if use_landmark_refinement:
            print(f'[INFO] Using BFM + {landmark_method} landmark refinement on GPU {gpu_id}')
        else:
            print(f'[INFO] Using traditional BFM without landmark refinement on GPU {gpu_id}')
    ...
    if use_deca:
        # Use DECA-based tracker
        refinement_flag = '--use_landmark_refinement' if use_landmark_refinement else ''
        cmd = f'python data_utils/face_tracking/face_tracker_deca.py --path={ori_imgs_dir} --img_h={h} --img_w={w} --frame_num={len(image_paths)} {refinement_flag} --landmark_method={landmark_method} --gpu_id={gpu_id}'
    else:
        # Use BFM-based tracker (enhanced version supports refinement)
        if use_landmark_refinement:
            refinement_flag = '--use_landmark_refinement'
            cmd = f'python data_utils/face_tracking/face_tracker_enhanced.py --path={ori_imgs_dir} --img_h={h} --img_w={w} --frame_num={len(image_paths)} {refinement_flag} --landmark_method={landmark_method} --gpu_id={gpu_id}'
        else:
            # Original BFM tracker (no refinement)
            cmd = f'python data_utils/face_tracking/face_tracker.py --path={ori_imgs_dir} --img_h={h} --img_w={w} --frame_num={len(image_paths)} --gpu_id={gpu_id}'
...
def save_transforms(base_dir, ori_imgs_dir):
    ...
    print(f'[INFO] Face model type: {face_model_type}')
    ...
    for split in range(2):
        ...
        transform_dict['face_model_type'] = face_model_type
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_deca', action='store_true', help="use DECA instead of BFM for face tracking")
    parser.add_argument('--use_landmark_refinement', action='store_true', 
                       help="use learning-based landmark refinement (MediaPipe/EMOCA/PRNet) - works with BOTH BFM and DECA")
    parser.add_argument('--landmark_method', type=str, default='mediapipe', 
                       choices=['mediapipe', 'emoca', 'prnet'],
                       help="landmark refinement method: mediapipe (fastest), emoca (emotion-aware), or prnet (highest detail)")
    parser.add_argument('--gpu_id', type=int, nargs='+', default=[0], 
                       help="GPU IDs to use. Single value for all tasks, or list [gpu_main, gpu_face_tracking] e.g., --gpu_id 0 1")

    opt = parser.parse_args()
    
    # Parse GPU IDs
    if len(opt.gpu_id) == 1:
        # Single GPU for all tasks
        gpu_main = opt.gpu_id[0]
        gpu_face_tracking = opt.gpu_id[0]
    elif len(opt.gpu_id) == 2:
        # First GPU for main tasks, second GPU for face tracking
        gpu_main = opt.gpu_id[0]
        gpu_face_tracking = opt.gpu_id[1]
        print(f"[INFO] Using GPU {gpu_main} for main tasks and GPU {gpu_face_tracking} for face tracking")
    else:
        raise ValueError("--gpu_id accepts 1 or 2 values: [gpu_for_all] or [gpu_main, gpu_face_tracking]")
    
    # Set main GPU for torch operations
    if torch.cuda.is_available():
        torch.cuda.set_device(gpu_main)
        print(f"[INFO] Main GPU set to: {gpu_main}")
    ...
    if opt.task == -1 or opt.task == 7:
        face_tracking(ori_imgs_dir, use_deca=opt.use_deca, use_landmark_refinement=opt.use_landmark_refinement, landmark_method=opt.landmark_method, gpu_id=gpu_face_tracking)
