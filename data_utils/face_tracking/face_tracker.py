...
parser.add_argument('--gpu_id', type=int, default=0, help='GPU ID to use (e.g., 0, 1, 2, etc.)')
...
model_3dmm = Face_3DMM(os.path.join(dir_path, '3DMM'),
                       id_dim, exp_dim, tex_dim, point_num, device=device)
lands_info = np.loadtxt(os.path.join(
    dir_path, '3DMM', 'lands_info.txt'), dtype=np.int32)
lands_info = torch.as_tensor(lands_info).to(device)
...
torch.save({'id': id_para.detach().cpu(), 'exp': exp_para.detach().cpu(),
            ...
            'face_model_type': 'BFM'}, os.path.join(os.path.dirname(args.path), 'track_params.pt'))
print('face tracking params saved (BFM model)')
