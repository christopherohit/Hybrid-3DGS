...
class Face_3DMM(nn.Module):
    def __init__(self, modelpath, id_dim, exp_dim, tex_dim, point_num, device='cuda'):
        ...
        for i in range(3):
            mu[:, i] -= np.mean(mu[:, i])
        mu = mu.reshape(-1)
        self.base_id = torch.as_tensor(base_id).to(device) /1000.0
        self.base_exp = torch.as_tensor(base_exp).to(device) /1000.0
        self.mu = torch.as_tensor(mu).to(device) /1000.0
