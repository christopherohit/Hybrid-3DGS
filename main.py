...
if __name__ == '__main__':
    ...
    parser.add_argument('--landmark_backend', type=str, default='fan', choices=['dlib', 'fan', 'mediapipe'], help="backend for landmark detection (dlib, fan/face_alignment, or mediapipe)")
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            metrics = [PSNRMeter(), LPIPSMeter(device=device), LMDMeter(backend=opt.landmark_backend)]
        ...
        if opt.test_train:
            ...
            print(f'[INFO] ===== Testing with face model: {test_set.face_model_type} =====')
        else:
            test_dataset = NeRFDataset(opt, device=device, type='test')
            test_loader = test_dataset.dataloader()
            print(f'[INFO] ===== Testing with face model: {test_dataset.face_model_type} =====')
        ...
        # Display which face model was used for tracking
        print(f'[INFO] ===== Training with face model: {train_dataset.face_model_type} =====')
        ...
        # Save training configuration including face model type
        with open(os.path.join(opt.workspace, 'opt.txt'), 'a') as f:
            f.write(str(opt))
            f.write(f'\n\nFace Model Type: {train_dataset.face_model_type}\n')
        if opt.gui:
